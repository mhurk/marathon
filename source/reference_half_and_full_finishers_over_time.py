...
#### Reference Marathon Eindhoven with old start times
# same number of runners as 2024
# same pace
# same waves
# start full at 10:00 and half at 14:00 (delta = 240 minutes)
# gpx from full from 2017, this is the same as 2023 and half from 2023. (not for this analysis)
...
max_time = 500 # Maximum time in minutes
...
start_offset = 240 #half marathon starts 240 minutes after the full
...
def update(frame):
    ...
    ax5.set_title('Cumulative finishers - full and half marathon reference')
...
ani.save('../images/wave_simulation_animation_finishers_reference.mp4', writer='ffmpeg', fps=10)

# Store last frame
update(len(time_steps) - 1)  # Update to the last frame
plt.savefig('../images/wave_simulation_finishers_last_frame_reference.png', dpi=300)
